...
void qCorrectMag(gsl_vector_float *state, const gsl_vector_float *mag) {
  ...
  gsl_matrix_float_set(R, 1, 0, gsl_matrix_float_get(R, 0, 1));
  gsl_matrix_float_set(R, 2, 0, gsl_matrix_float_get(R, 0, 2));
  gsl_matrix_float_set(R, 2, 1, gsl_matrix_float_get(R, 1, 2));
  ...
  if (tr > 0) {
    ...
    if (d0 >= d1 && d0 >= d2) {
      float s = 2 * sqrt(1 + d0 - d1 - d2);
      qw = (R32 - R23) / s;
      qx = 0.25 * s;
      qy = (R12 + R21) / s;
      qz = (R13 + R31) / s;
    } else if (d1 >= d2) {
      float s = 2 * sqrt(1 + d1 - d0 - d2);
      qw = (R13 - R31) / s;
      qx = (R12 + R21) / s;
      qy = 0.25 * s;
      qz = (R23 + R32) / s;
    } else {
      float s = 2 * sqrt(1 + d2 - d0 - d1);
      qw = (R21 - R12) / s;
      qx = (R13 + R31) / s;
      qy = (R23 + R32) / s;
      qz = 0.25 * s;
...
void PCorrectMag(const gsl_vector_float *x, const gsl_vector_float *mag, const gsl_vector_float *sigma_mag,
                  gsl_matrix_float *P) {
  ...
  gsl_blas_sgemm(CblasNoTrans, CblasNoTrans, 1.0, &J_4_3.matrix, sigma, 0.0, tmp4_3);
