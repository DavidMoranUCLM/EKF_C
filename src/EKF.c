...
void ekfInit(EKF_ctx_t* ctx, const measures_t* measures) {
  ...
  ctx->currentTime = 0;
  ctx->prevTime = 0;
  ctx->lastMagCorrectionTime_s = 0;
  ctx->magStdDev = gsl_vector_float_calloc(3);
  gsl_vector_float_set(ctx->magStdDev, 0, MAG_STD_DEVIATION);
  gsl_vector_float_set(ctx->magStdDev, 1, MAG_STD_DEVIATION);
  gsl_vector_float_set(ctx->magStdDev, 2, MAG_STD_DEVIATION);
...
void initWorkSpace(EKF_ctx_t* ctx) {
  ...
  wk->H = gsl_matrix_float_calloc(3, 4);
  wk->K = gsl_matrix_float_calloc(4, 3);
  wk->R = gsl_matrix_float_calloc(3, 3);
  wk->S = gsl_matrix_float_calloc(3, 3);
  wk->invS = gsl_matrix_float_calloc(3, 3);
  wk->doubleS = gsl_matrix_calloc(3, 3);
  wk->doubleInvS = gsl_matrix_calloc(3, 3);
  ...
  // NEW: Allocate double-precision buffers for QR inversion (size 3x3)
  wk->inv_tmpMatrix_d = gsl_matrix_alloc(3, 3);
  wk->inv_tmpTau_d = gsl_vector_alloc(3);
  wk->inv_tmpB_d = gsl_vector_alloc(3);
  wk->inv_tmpX_d = gsl_vector_alloc(3);
  ...
  // NEW: Allocate tmpRTransK (3x4) for R*Trans(K)
  wk->tmpRTransK = gsl_matrix_float_calloc(3, 4);
}
...
void get_h(EKF_ctx_t* ctx) {
  EKF_work_ctx_t* wk = ctx->wk;
  get_hPrimitive(ctx->q_est, ctx->horizonRefG, wk->h);
}
...
int getH(EKF_ctx_t* ctx) {
  ...
  getHPrimitive(ctx->q_est, ctx->horizonRefG, wk->H,
...
void PCorrectPrimitive(const gsl_matrix_float* P, const gsl_matrix_float* K,
  ...
  if (P_CORRECT_METHOD == 0) {
    ...
    gsl_blas_sgemm(CblasNoTrans, CblasTrans, 1, R, K, 0, tmp3x4);
    gsl_blas_sgemm(CblasNoTrans, CblasNoTrans, 1, K, tmp3x4, 0, tmp4x4);
...
void getKPrimitive(const gsl_matrix_float* P, const gsl_matrix_float* H,
                   const gsl_matrix_float* invS, gsl_matrix_float* K,
                   gsl_matrix_float* tmp4x3) {
  // Use provided tmp4x3 instead of allocating it.
  gsl_blas_sgemm(CblasTrans, CblasNoTrans, 1, H, invS, 0,
                 tmp4x3);  // tmp4x3 = trans(H)*invS
  gsl_blas_sgemm(CblasNoTrans, CblasNoTrans, 1, P, tmp4x3, 0,
                 K);  // K = P * tmp4x3
}
...
void getSPrimitive(const gsl_matrix_float* H, const gsl_matrix_float* P,
  ...
  gsl_blas_sgemm(CblasNoTrans, CblasTrans, 1, P, H, 0,
                 tmp4x3);  // tmp4x3 = P*trans(H)
  gsl_blas_sgemm(CblasNoTrans, CblasNoTrans, 1, H, tmp4x3, 0,
                 S);           // S = H*tmp4x3
